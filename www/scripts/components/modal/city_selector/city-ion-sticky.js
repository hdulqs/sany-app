...
angular.module('huilianyi.pages')
    .directive('ionStickyCity', ['$ionicPosition', '$compile', '$timeout', function ($ionicPosition, $compile, $timeout) {
        return {
            restrict: 'A',
            require: '^$ionicScroll',
            link: function ($scope, $element, $attr, $ionicScroll) {
                ...
                var updateSticky = ionic.throttle(function() {
                    ...
                    for (var i = 0; i < dividers.length; ++i) { // can be changed to binary search
                        if(i === 0){
                            if ($ionicPosition.offset(dividers[i]).top - $ionicPosition.offset($element).top < 0) { // this equals to jquery outerHeight
                                if (i === dividers.length-1 || $ionicPosition.offset(dividers[i+1]).top -
                                    ($ionicPosition.offset($element).top + dividers[i+1].prop('offsetHeight')) > 0) { //when clone covers new header
                                    active = dividers[i][0];
                                    break;
                                }
                            }
