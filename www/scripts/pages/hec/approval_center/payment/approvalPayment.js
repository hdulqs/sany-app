...
angular.module('huilianyi.pages')
    .config(['$stateProvider', function ($stateProvider) {
        $stateProvider
            .state('app.approvalPayment', {
                ...
                views: {
                    'page-content@app': {
                        templateUrl: 'scripts/pages/hec/approval_center/payment/approvalPayment.html',
    ...
    .controller('approvalPaymentController', ['$scope', '$filter', '$ionicHistory', '$document', 'Auth', '$state', '$ionicLoading', '$timeout', '$ionicPopup', 'approvalService', 'PublicFunction', '$stateParams',
        'LocalStorageKeys', '$ionicModal', 'HecbempService', 'approvalRecordService', 'HecImageService', 'PageValueService',
        function ($scope, $filter, $ionicHistory, $document, Auth, $state, $ionicLoading, $timeout, $ionicPopup, approvalService, PublicFunction, $stateParams
            , LocalStorageKeys, $ionicModal, HecbempService, approvalRecordService, HecImageService, PageValueService) {
            var vm = this;
            //初始化page
            vm.initPage = initPage;
            //付款申请行详情
            vm.goPaymentLine = goPaymentLine;
            //查看关联的报销单
            vm.goRefReport = goRefReport;
            //拒绝审批
            ...
            //参数
            vm.appCenterPayment = PageValueService.get("appCenterPayment");
            vm.type = vm.appCenterPayment.type;//用来判断是从审批中心(approval)跳转,还是其他调转
            vm.passFlag = vm.appCenterPayment.passFlag;
            vm.status = vm.appCenterPayment.status;
            vm.approvalPayment = vm.appCenterPayment.approvalPayment;
            if (vm.approvalPayment != null) {
                vm.headerId = vm.approvalPayment.instance_param;
                vm.recordId = vm.approvalPayment.record_id;
                vm.instanceId = vm.approvalPayment.instance_id;
            ...
            function initPage() {
                ...
                //加载附件
                HecImageService.downloadImage("prh", vm.headerId).then(function (res) {
                    vm.attachments = res;
                });
                //获取付款单头
                approvalService.searchApprovalPayment('center_acp_requisition_head', vm.headerId).then((function (res) {
                    if (res.data.success) {
                        if(res.data.result.pageCount > 0){
                            vm.header = res.data.result.record[0];
                            //获取付款单行
                            approvalService.searchApprovalPayment('center_acp_requisition_line', vm.headerId).then((function (res) {
                                if (res.data.success && res.data.result.pageCount > 0) {
                                    ...
                                    for (var i = 0; i < vm.line.length; i++) {
                                        vm.header.total_amount = vm.header.total_amount + vm.line[i].req_amount;
                                        //行币种
                                        vm.line[i].currency_code = vm.header.currency_code;
                                    }
                                    ...
                                    approvalService.getActionId(vm.recordId).then((function (res) {
                                        if (res.data.success) {
                                            ...
                                            if (pageCnt > 0) {
                                                ...
                                                //获取单据跟踪记录
                                                approvalRecordService.paymentRecord(vm.headerId).then(function (res) {
                                                    if (res.data.success) {
                                                        if(res.data.result.pageCount > 0){
                                                            vm.approvalRecordList = res.data.result.record;
                                                        }
                                                    ...
                                                    PublicFunction.showToast($filter('translate')('error.get.payment.record.failed'));//获取付款单单据跟踪记录失败!
            ...
            /**
             * 查看付款单行详情
             * @param itemLine 付款单对象
             * @param type 跳转类型：approval:审批中心模块、req申请单模块
             */
            function goPaymentLine(item) {
                var params = {
                    item: item
                }
                $state.go('app.paymentItem', params);
            };

            /**
             * 跳转关联报销单页面
             * @param expHeaderId 关联报销单id
             */
            function goRefReport(expHeaderId) {
                if (PublicFunction.isNull(expHeaderId)) {
                    PublicFunction.showToast($filter('translate')('message.error.query'));  // 数据查询失败
                } else {
                    var params = {
                        "status": "hasApproval",
                        "approvalReport": {instance_param: expHeaderId},
                        "type": 'approval'
                    };
            ...
            function refuseApproval() {
                ...
                approvalService.refuseApproval(vm.recordId, vm.refuseActionId, vm.approvalText).then(function (res) {
                    ...
                    if (res.data.success) {
                        console.log("已拒绝");
                        if (vm.appCenterPayment.messageFlag === 'Y') {
            ...
            $scope.goBack = function () {
                PageValueService.set("appCenterPayment", {});
                if (vm.appCenterPayment.messageFlag === 'Y') {
                    $state.go('app.erv_notification');
                } else {
