...
angular.module('huilianyi.pages')
    .config(['$stateProvider', function ($stateProvider) {
        $stateProvider
            .state('app.travelReqHeader', {
                ...
                views: {
                    'page-content@app': {
                        templateUrl: 'scripts/pages/hec/expense_req/travel/travelReqHeader.html',
                        controller: 'RtravelReqHeaderrCtrl',
                        controllerAs: 'vm'
                    }
                },
                resolve: {
    ...
    .controller('RtravelReqHeaderrCtrl', ['$scope', '$q', '$ionicPopup', '$cordovaDatePicker', 'LANG', '$filter', '$ionicHistory', '$state', '$ionicLoading', '$timeout',
        ...
        function ($scope, $q, $ionicPopup, $cordovaDatePicker, LANG, $filter, $ionicHistory, $state, $ionicLoading, $timeout,
            ...
            vm.isReadOnly = false;
            vm.stateFlag = ""; // Y/国际差旅申请单   N/国内差旅申请单
            ...
            vm.toTravelLineDetail = toTravelLineDetail;
            ...
            /**
             * 初始化页面(vm.reqHeaderId不为空则查询，为空则调用差旅申请单头初始化接口)
             */
            function initPage() {
                //安卓，从行上点击返回键时，加载页面数据
                ...
                if (PublicFunction.isNull(vm.reqHeaderId)) {
                    ...
                    ReqHeaderService.searchTravelHeader(vm.reqHeaderId).then(function (res) {
                        $ionicLoading.hide();
                        if (res.data.success) {
                            ...
                            PublicFunction.showToast(res.data.error.message);  // 差旅申请头查询失败
                        }
                    }, function (error) {
                        $ionicLoading.hide();
            ...
            function goDocHistory() {
                $state.go('app.docHistory', {type: 'travelReq', headerId: vm.reqHeaderId});
            }
            ...
            /**
             * 计算差旅行的费用总金额和格式化出差日期和出差地
             */
            function calTotalAmount() {
                var amount = 0;
                angular.forEach(vm.lineList, function (line) {
                    line.line_amount = PublicFunction.str2Number(line.fly_requisition_amount) + PublicFunction.str2Number(line.hotel_requisition_amount)
                        + PublicFunction.str2Number(line.other_requisition_amount);
                    amount = amount + line.line_amount;
                    line.from_to = line.place_from + " - " + line.place_to;
                    line.travel_date = line.date_from + " - " + line.date_to;
                    if (PublicFunction.compareDate(new Date(), line.date_from)) {
                        vm.earlyFlag = true;
                    }
                });
            ...
            function initTravelHeader() {
                ReqHeaderService.initTravelHeader(vm.comDocData.employeeId, vm.comDocData.companyId,vm.comDocData.docTypeId).then(function (res) {
                    if (res.data.success) {
                        ...
                        PublicFunction.showToast(res.data.error.message);  // 差旅申请头初始化失败
                    }
                }, function (error) {
            ...
            function deleteReqLine(line) {
                vm.saveReqHeader("").then(function (data) {
                    if (data === "S") {
                        //调用差旅申请行删除接口
                        PublicFunction.showLoading(250);
                        ReqHeaderService.deleteTravelReqLine(vm.reqHeaderId, line.req_travel_id).success(function (res) {
                            if (res.success) {
                                $ionicLoading.hide();
                                //计算总金额
                                var amount =  PublicFunction.str2Number(line.fly_requisition_amount)
                                    + PublicFunction.str2Number(line.hotel_requisition_amount)
                                    + PublicFunction.str2Number(line.other_requisition_amount);
                                vm.header.total_amount = vm.header.total_amount - amount;
                                //删除
            ...
            function preSubmit() {
                ...
                if(vm.reqHeaderId){
                    //调用需维护差旅申请人接口
                    ReqHeaderService.submitBeneficiaryCheck(vm.reqHeaderId).success(function (res) {
                        if (res.success) {
                            if (res.result&&res.result.beneficiary_flag=="Y") {
                                deferred.resolve("S");
                            }else if(res.result&&res.result.beneficiary_flag=="N"){
                                $ionicPopup.confirm({
                                    title: $filter('translate')('req.header.submit.tip'),
                                    template: '<p style="text-align: center">' + $filter('translate')('req.message.error.line.required.one.applicant') + '</p>',
                                    ...
                                    if (result) {
                                        if (vm.earlyFlag) {
                                            $ionicPopup.confirm({
                                                title: $filter('translate')('req.header.submit.tip'),
                                                template: '<p style="text-align: center">' + $filter('translate')('req.message.error.header.exist.early.current.date') + '</p>',
                                                cancelText: $filter('translate')('system_check.cancel'),
                                                cancelType: 'button-calm',
                                                okText: $filter('translate')('system_check.ok')
                                            }).then(function (result) {
            ...
            /**
             * 查看差旅申请行信息(先保存头信息再跳转到行界面)
             * @param reqLineId 差旅申请行ID
             */
            function toTravelLineDetail(reqLineId) {
                vm.saveReqHeader("").then(function (data) {
                    if (data === "S") {
                        PageValueService.set("reqHeader", "");
                        ...
                        $state.go('app.travelReqLine', {
                            reqHeaderId: vm.reqHeaderId,
                            reqLineId: reqLineId,
                            maxLineNum: vm.header.travel_max_num
                        });
                    }
                });
            }
            ...
            $scope.showPopup = function () {
                var confirmPopup = $ionicPopup.confirm({
                    ...
                    template: $filter('translate')('confirm.window.Information.travel'),//是否保存该差旅申请单
